...
Simple assumes the machine performing the check is **friendly** you control
where the client runs. The protocol is stateless —
...
- Typed errors distinguish infrastructure failures from license denials, so
  ...
  `AuthenticateWith*Async` returns `true` only for a literal `true` answer.

## Keep in mind

- HWID locking defaults to the fault-tolerant SL-HWID mode (see below). The
  opt-in legacy hash (`HwidMode = "legacy"`) is exact-match: it changes when
...
The default threshold HWID mode (`HwidMode = "sl-hwid"`) makes copied state
...
deterministically after use, matching the rest of this client; managed
memory makes that best-effort rather than absolute.
